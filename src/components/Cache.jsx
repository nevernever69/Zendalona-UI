...
const Cache = () => {
  ...
  const handleUpdateCacheEntry = async (entryId) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`https://chatapi.zendalona.com/cache/update/${entryId}`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleImportCsv = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://chatapi.zendalona.com/cache/import', {
        method: 'POST',
        body: formData,
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
  ...
  const handleExportCsv = async () => {
    try {
      const response = await fetch('https://chatapi.zendalona.com/cache/export');
      if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
